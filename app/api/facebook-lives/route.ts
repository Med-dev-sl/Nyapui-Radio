...
export async function GET() {
  try {
    const [rows]: any = await db.execute(
      `SELECT f.id, f.facebook_url, f.topic, f.guest_name, f.guest_position, f.guest_institution,
...
export async function POST(request: Request) {
  try {
    ...
    const [result]: any = await db.execute(
      `INSERT INTO facebook_lives (facebook_url, topic, guest_name, guest_position, guest_institution, presenter_name)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [facebook_url, topic || null, guest_name || "", guest_position || "", guest_institution || "", presenter_name || ""]
    );
