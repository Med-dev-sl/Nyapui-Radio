...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await db.execute(
      `UPDATE facebook_lives SET facebook_url = ?, guest_name = ?, guest_position = ?, guest_institution = ?, presenter_name = ? WHERE id = ?`,
      [facebook_url, guest_name || "", guest_position || "", guest_institution || "", presenter_name || "", id]
    );
